...
public class ElasticSearchBulkDialog extends BaseStepDialog implements StepDialogInterface {
  ...
  private static final Logger LOG = LogManager.getLogger(ElasticSearchBulkDialog.class);
  ...
  public void getData( ElasticSearchBulkMeta in ) {
    ...
    // Servers
    for ( TransportAddress server : model.getServers() ) {
...
/*    // Save off the thread's context class loader to restore after the test
    ...
    Node node = null;
    PreBuiltHWTransportClient tClient = null;
    try {

      ElasticSearchBulkMeta tempMeta = new ElasticSearchBulkMeta();
      toModel( tempMeta );
      // 设置jdk的 configuration 为空
      javax.security.auth.login.Configuration.setConfiguration(null);

      Settings.Builder settingsBuilder = Settings.builder();
      settingsBuilder.put( Settings.Builder.EMPTY_SETTINGS ); // keep default classloader
      Iterator<Entry<String, String>> items = tempMeta.getSettings().entrySet().iterator();
      while (items.hasNext()){
        Entry<String, String> item = items.next();
        settingsBuilder.put( item.getKey(), transMeta
            .environmentSubstitute( item.getValue() ));
      }
      if ( tempMeta.getServers().length > 0 ) {
        node = null;
        Configuration configuration = new Configuration();
        String isSecureMode = "false";
        boolean sniff = false;
        if(settingsBuilder.get("isSecureMode")!=null){
          isSecureMode = settingsBuilder.get("isSecureMode");
        }
        if(settingsBuilder.get("isSniff")!=null){
          sniff = Boolean.valueOf(settingsBuilder.get("isSniff"));
        }
        String principal = settingsBuilder.get("principal");
        configuration.setClusterName(settingsBuilder.get("cluster.name"));
        Set<TransportAddress> transportAddresses = new HashSet<>(tempMeta.getServers().length);
        for ( TransportAddress address : tempMeta.getServers() ) {
          transportAddresses.add( new TransportAddress(
              InetAddress.getByName( transMeta.environmentSubstitute( address.getAddress() ) ),
              address.getPort() ) );
        }
        configuration.setTransportAddress(transportAddresses);
        configuration.setSecureMode(Boolean.valueOf(isSecureMode));
        ...
        if (configuration.isSecureMode()) {
          ...
          LoadProperties.setSecurityConfig(configuration.getPrincipal(), configPath, configPath);
        }
        configuration.setSniff(sniff);
        ClientFactory.initConfiguration(configuration);
        tClient = ClientFactory.getClient();
      ...
      switch ( testType ) {
        case INDEX:
          ...
          IndicesExistsRequestBuilder indicesExistBld = tClient.prepare().admin().indices().prepareExists( "test");
          ...
          RecoveryRequestBuilder indicesBld = tClient.prepare().admin().indices().prepareRecoveries( "test");
